defmodule ElixircnWeb.Components.UI.Icon do
  @icons %{
    ...
    "chevrons-down-up" => ~s(<path d="m7 20 5-5 5 5" /> <path d="m7 4 5 5 5-5" />),
    "chevrons-up-down" => ~s(<path d="m7 15 5 5 5-5" /> <path d="m7 9 5-5 5 5" />),
    "circle-alert" =>
      ~s(<circle cx="12" cy="12" r="10" /> <line x1="12" x2="12" y1="8" y2="12" /> <line x1="12" x2="12.01" y1="16" y2="16" />),
    "circle-x" =>
      ~s(<circle cx="12" cy="12" r="10" /> <path d="m15 9-6 6" /> <path d="m9 9 6 6" />),
    "copy" =>
    ...
    "file" =>
      ~s(<path d="M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z" /> <path d="M14 2v5a1 1 0 0 0 1 1h5" />),
    "file-edit" =>
      ~s(<path d="M12.659 22H18a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v9.34" /> <path d="M14 2v5a1 1 0 0 0 1 1h5" /> <path d="M10.378 12.622a1 1 0 0 1 3 3.003L8.36 20.637a2 2 0 0 1-.854.506l-2.867.837a.5.5 0 0 1-.62-.62l.836-2.869a2 2 0 0 1 .506-.853z" />),
    "file-text" =>
    ...
    "grip-horizontal" =>
      ~s(<circle cx="12" cy="9" r="1" /> <circle cx="19" cy="9" r="1" /> <circle cx="5" cy="9" r="1" /> <circle cx="12" cy="15" r="1" /> <circle cx="19" cy="15" r="1" /> <circle cx="5" cy="15" r="1" />),
    "grip-vertical" =>
      ~s(<circle cx="9" cy="12" r="1" /> <circle cx="9" cy="5" r="1" /> <circle cx="9" cy="19" r="1" /> <circle cx="15" cy="12" r="1" /> <circle cx="15" cy="5" r="1" /> <circle cx="15" cy="19" r="1" />),
    "grid-2x2" =>
      ~s(<path d="M12 3v18" /> <path d="M3 12h18" /> <rect x="3" y="3" width="18" height="18" rx="2" />),
    "info" =>
    ...
    "log-out" =>
      ~s(<path d="m16 17 5-5-5-5" /> <path d="M21 12H9" /> <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" />),
    "monitor" =>
    ...
    "search" => ~s(<path d="m21 21-4.34-4.34" /> <circle cx="11" cy="11" r="8" />),
    "settings" =>
  ...
  @moduledoc """
  SVG icon component powered by a built-in Lucide icon set.

  ## Available icons

  #{Enum.join(@icon_names, ", ")}

  ## Sizing

  The default size is `size-4` (1rem × 1rem). Override via the `class` attribute —
  later classes win when there is a Tailwind conflict:

      <.icon name="check" class="size-6" />
      <.icon name="check" class="size-[1em]" />  # match surrounding text size

  ## Adding custom icons

  Register custom icons in your application config:

      # config/config.exs (or runtime.exs)
      config :elixircn, :custom_icons, %{
        "circle-alert" => ~s(<circle cx="12" cy="12" r="10"/>
          <line x1="12" x2="12" y1="8" y2="12"/>
          <line x1="12" x2="12.01" y1="16" y2="16"/>)
      }

  Custom icons are looked up at runtime, so no recompile is needed when
  they are added via runtime config.
  """
  ...
  @doc "Renders an SVG icon by name from the built-in Lucide icon library."
  def icon(assigns) do
    ~H"""
    <svg
  ...
  for {name, svg} <- @icons do
    defp icon_svg(unquote(name)), do: unquote(svg)
  end

  defp icon_svg(name) do
    custom = Application.get_env(:elixircn, :custom_icons, %{})

    case Map.fetch(custom, name) do
      {:ok, svg} ->
        svg

      :error ->
        raise ArgumentError, """
        Icon #{inspect(name)} not found.

        Built-in icons: #{Enum.join(@icon_names, ", ")}

        To add a custom icon, configure:
